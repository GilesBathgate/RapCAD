# Agent Development Workflow version 2.4
...
```bash
rm -vf /app/build/rapcad /app/build/lib/librapcad.a
```
...
```bash
make -j$(nproc) -C /app/build > /app/build/build.log 2>&1 &
```
...
```bash
/app/build/test/test --test /app/test/assets
```
